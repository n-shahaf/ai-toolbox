...
// Agents:  .cursor/rules/<name>.mdc        (manually invoked by `@`-mention)
// Skills:  .cursor/skills/<skill-name>/SKILL.md   (referenced by ../skills/... from agents)
const cursor = {
  name: 'cursor',
  description: 'Cursor IDE — .cursor/rules (and .cursor/skills)',
  agentDest: (src) => `.cursor/rules/${agentBaseName(src)}.mdc`,
  skillDest: (src) => `.cursor/skills/${skillDir(src)}/SKILL.md`,
  transformAgent(text) {
    // Cursor rules accept: description, globs, alwaysApply. Strip Copilot's model/tools.
    // alwaysApply: false so the user opts in by @-mentioning the rule — forcing all
    // agents to always-apply would dump every role's prompt into every turn.
    const { frontmatter, body } = parseFrontmatter(text);
    const out = {};
    if (frontmatter.description) out.description = frontmatter.description;
...
export function getTarget(name, opts = {}) {
  ...
  throw new Error(`unknown target: ${name} (expected: claude | copilot | cursor | generic)`);
}
